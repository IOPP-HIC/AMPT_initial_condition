...
C
        ...
        COMMON/hjwsxro/YP_D(3,300),YT_D(3,300)
cc      SAVE /hjcrdn/
...
cc      SAVE /NOPREC/
cc  wuxy-  2018/5/25
        DOUBLE PRECISION  He3_xtot(10000,3,3),He3_rotation(10000,3,3)
c     &       He3_ytot(0:100000,0:3,0:3),He3_ztot(0:100000,0:3,0:3)
        integer index_i,Nhe3,he3_i
cc  wuxy- 2023/3/11
        DOUBLE PRECISION  d_Ox(20000,16,3),d_O0(16,3),d_O1(16,3)
        integer N_oxygen,i_oxygen

        DOUBLE PRECISION  d_Nex(20000,20,3),d_Ne0(20,3),d_Ne1(20,3)
        integer N_Ne,i_Ne
...
cc      SAVE /lastt/
        ...
        common/oscar1/iap,izp,iat,izt,ndf,nialpha
cc      add by wxy
...
C
        ...
        IF(IHNT2(1).EQ.2) GO TO 6614
        IF(IHNT2(1).EQ.3) GO TO 6615
        IF(IHNT2(1).EQ.16 .AND. nialpha .GT. 0 ) GO TO 6616
        IF(IHNT2(1).EQ.20 .AND. nialpha .GT. 0 ) GO TO 6617

        
        IF(ABS(ndf-0).LE.0.00001) THEN
            DO 10 KP=1,IHNT2(1)
5            R=HIRND(1)
            X=RANART(NSEED)
            CX=2.0*X-1.0
...
10            CONTINUE
        ...
        IF(ABS(ndf-1).LE.0.00001) THEN
        
c        open(60,FILE='particles.dat',STATUS='UNKNOWN')
        do 500 KP=1,IHNT2(1)
501            do while(.TRUE.)
                R=HIPR1(34)*(RANART(NSEED))**(1./3.)
                CX = 2.0*RANART(NSEED)-1.0
                y_rand=RANART(NSEED)        
                if((WDSAX_DE(R,CX)-y_rand)>1.0e-20) exit
            end do
        
            SX=SQRT(1.0-CX*CX)
            PHI=RANART(NSEED)*2.0*HIPR1(40)
     
            YP_D(1,KP)=R*SX*COS(PHI)
            YP_D(2,KP)=R*SX*SIN(PHI)
            YP_D(3,KP)=R*CX
c            write(60,*) x_d,y_d,z_d
            IF(HIPR1(29).EQ.0.0) GO TO 500 
            do KP2=1,KP-1
                DNBP1=(YP_D(1,KP)-YP_D(1,KP2))**2
                DNBP2=(YP_D(2,KP)-YP_D(2,KP2))**2
                DNBP3=(YP_D(3,KP)-YP_D(3,KP2))**2
                DNBP=DNBP1+DNBP2+DNBP3
                IF(DNBP.LT.HIPR1(29)*HIPR1(29)) GO TO 501
            enddo
500        continue 
c        close(60)
        cos_a = 2.0*RANART(NSEED)-1.0
        sin_a = sqrt(1.0-cos_a*cos_a)
        
        angle_b = RANART(NSEED)*2.0*HIPR1(40)  
        cos_b = cos(angle_b)
        sin_b = sin(angle_b)
        
c        open(60,FILE='particles.dat',STATUS='UNKNOWN')
        do KP=1,IHNT2(1)
            YP(1,KP)=YP_D(1,KP)*cos_b-YP_D(2,KP)*cos_a*sin_b
     &               +YP_D(3,KP)*sin_a*sin_b

            YP(2,KP)=YP_D(1,KP)*sin_b+YP_D(2,KP)*cos_a*cos_b
     &               -YP_D(3,KP)*sin_a*cos_b
            YP(3,KP)=YP_D(2,KP)*sin_a+YP_D(3,KP)*cos_a
c            write(60,*) YP(1,KP),YP(2,KP),YP(3,KP)
        enddo
c        close(60)
        ENDIF
...
C          wuxy helium3
C*****************************
6615        if(IHNT2(1).EQ.3) then
           
           OPEN (525, FILE = './He3list.dat',
     &         STATUS = 'UNKNOWN')
          
           Nhe3=10000
           do index_i =1,Nhe3  
                READ(unit=525,fmt=*) He3_xtot(index_i,1,1),
     &         He3_xtot(index_i,1,2),
     &              He3_xtot(index_i,1,3),He3_xtot(index_i,2,1),
     &              He3_xtot(index_i,2,2),He3_xtot(index_i,2,3),
     &              He3_xtot(index_i,3,1),He3_xtot(index_i,3,2),
     &              He3_xtot(index_i,3,3)
                
           end do
           CLOSE(525)
        
           do index_i = 1,Nhe3
                ang1 = RANART(NSEED)*2.0*HIPR1(40)
                c1 = cos(ang1)
                s1 = sin(ang1)

                c2 = 2.0* RANART(NSEED) -1 
                s2 = sqrt(1. - c2*c2)

                ang3 = RANART(NSEED)*2.0*HIPR1(40)
                c3 = cos(ang3)
                s3 = sin(ang3)
                
                do index_j = 1,3
                        He3_rotation(index_i,index_j,1)
     &                  = He3_xtot(index_i,index_j,1)*(c1*c3-c2*s1*s3) 
     &                  - He3_xtot(index_i,index_j,2)*(c3*s1+c1*c2*s3) 
     &                  + He3_xtot(index_i,index_j,3)*(s2*s3)
     
                        He3_rotation(index_i,index_j,2)   
     &                  = He3_xtot(index_i,index_j,1)*(c1*s3+c2*c3*s1) 
     &                  - He3_xtot(index_i,index_j,2)*(s1*s3-c1*c2*c3) 
     &                  - He3_xtot(index_i,index_j,3)*(c3*s2)

                        He3_rotation(index_i,index_j,3)   
     &                  = He3_xtot(index_i,index_j,1)*(s1*s2) 
     &                  + He3_xtot(index_i,index_j,2)*(c1*s2) 
     &                  + He3_xtot(index_i,index_j,3)*(c2)
                enddo
           ...
           YP(1,1)= He3_rotation(he3_i,1,1)
           YP(2,1)= He3_rotation(he3_i,1,2)
           YP(3,1)= He3_rotation(he3_i,1,3)
         
           YP(1,2)= He3_rotation(he3_i,2,1)
           YP(2,2)= He3_rotation(he3_i,2,2)
           YP(3,2)= He3_rotation(he3_i,2,3)

           YP(1,3)= He3_rotation(he3_i,3,1)
           YP(2,3)= He3_rotation(he3_i,3,2)
           YP(3,3)= He3_rotation(he3_i,3,3)
...
C          wuxy oxygen NLEFT+
C*****************************
6616        if(IHNT2(1).EQ.16 .AND. nialpha .GT. 0 ) then
           if (nialpha .EQ. 1) then
               OPEN (526, FILE = './NLEFT_dmin_0_5fm_positive_O.dat',
     &         STATUS = 'UNKNOWN')
          
           N_oxygen=14746
           endif

           if (nialpha .EQ. 2) then
               OPEN (526, FILE = './NLEFT_dmin_0_5fm_negative_O.dat',
     &         STATUS = 'UNKNOWN')
          
           N_oxygen=5254
           endif

           if (nialpha .EQ. 3) then
               OPEN (526, FILE = './PGCM_uniform_dmin0_O.dat',
     &         STATUS = 'UNKNOWN')
          
           N_oxygen=20000
           endif

           if (nialpha .EQ. 4) then
               OPEN (526, FILE = './PGCM_clustered_dmin0_O.dat',
     &         STATUS = 'UNKNOWN')
          
           N_oxygen=20000
           endif

           do i_oxygen =1,N_oxygen  
              do index_i = 1, IHNT2(3)
                 READ(unit=526,fmt=*) d_ox(i_oxygen,index_i,1),
     &              d_ox(i_oxygen,index_i,2),d_ox(i_oxygen,index_i,3)
           ...
           rnd1=max(RANART(NSEED),1.0e-20)   
           i_oxygen = rnd1*N_oxygen/1+1

           do index_i = 1, IHNT2(1)
                d_O0(index_i,1) = d_ox(i_oxygen,index_i,1)
                d_O0(index_i,2) = d_ox(i_oxygen,index_i,2)
                d_O0(index_i,3) = d_ox(i_oxygen,index_i,3)
           ...
           do index_i = 1, IHNT2(1)
               d_O1(index_i,1)
     &                  = d_O0(index_i,1)*(c1*c3-c2*s1*s3) 
     &                  - d_O0(index_i,2)*(c3*s1+c1*c2*s3) 
     &                  + d_O0(index_i,3)*(s2*s3)
     
               d_O1(index_i,2)   
     &                  = d_O0(index_i,1)*(c1*s3+c2*c3*s1) 
     &                  - d_O0(index_i,2)*(s1*s3-c1*c2*c3) 
     &                  - d_O0(index_i,3)*(c3*s2)

               d_O1(index_i,3)   
     &             = d_O0(index_i,1)*(s1*s2) 
     &             + d_O0(index_i,2)*(c1*s2) 
     &             + d_O0(index_i,3)*(c2)
           ...
           do index_i = 1, IHNT2(1)
              ...
              YP(1,index_i)= d_O1(index_i,1)
              YP(2,index_i)= d_O1(index_i,2)
              YP(3,index_i)= d_O1(index_i,3) 
...
C          wuxy Neon NLEFT+
C*****************************
6617        if(IHNT2(1).EQ.20 .AND. nialpha .GT. 0 ) then
           if (nialpha .EQ. 1) then
               OPEN (5301, FILE = './NLEFT_dmin_0_5fm_positive_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 2) then
               OPEN (5301, FILE = './NLEFT_dmin_0_5fm_negative_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 3) then
               OPEN (5301, FILE = './PGCM_uniform_dmin0_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 4) then
               OPEN (5301, FILE = './PGCM_clustered_dmin0_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           do i_Ne =1,N_Ne  
              do index_i = 1, IHNT2(1)
                 READ(unit=5301,fmt=*) d_Nex(i_Ne,index_i,1),
     &              d_Nex(i_Ne,index_i,2),d_Nex(i_Ne,index_i,3) 
           ...
           do index_i = 1, IHNT2(1)
                d_Ne0(index_i,1) = d_Nex(i_Ne,index_i,1)
                d_Ne0(index_i,2) = d_Nex(i_Ne,index_i,2)
                d_Ne0(index_i,3) = d_Nex(i_Ne,index_i,3)
           ...
           do index_i = 1, IHNT2(1)
               d_Ne1(index_i,1)
     &                  = d_Ne0(index_i,1)*(c1*c3-c2*s1*s3) 
     &                  - d_Ne0(index_i,2)*(c3*s1+c1*c2*s3) 
     &                  + d_Ne0(index_i,3)*(s2*s3)
     
               d_Ne1(index_i,2)   
     &                  = d_Ne0(index_i,1)*(c1*s3+c2*c3*s1) 
     &                  - d_Ne0(index_i,2)*(s1*s3-c1*c2*c3) 
     &                  - d_Ne0(index_i,3)*(c3*s2)

               d_Ne1(index_i,3)   
     &             = d_Ne0(index_i,1)*(s1*s2) 
     &             + d_Ne0(index_i,2)*(c1*s2) 
     &             + d_Ne0(index_i,3)*(c2)
           ...
           do index_i = 1, IHNT2(1)
              ...
              YP(1,index_i)= d_Ne1(index_i,1)
              YP(2,index_i)= d_Ne1(index_i,2)
              YP(3,index_i)= d_Ne1(index_i,3) 
...
14        YT(1,1)=0.0
        ...
        IF(IHNT2(3).EQ.2) GO TO 6624
        IF(IHNT2(3).EQ.3) GO TO 6625
        IF(IHNT2(3).EQ.16 .AND. nialpha .GT. 0 ) GO TO 6626
        IF(IHNT2(3).EQ.20 .AND. nialpha .GT. 0 ) GO TO 6627
...
503            do while(.TRUE.)
            ...
            YT_D(1,KT)=R*SX*COS(PHI)
            YT_D(2,KT)=R*SX*SIN(PHI)
            YT_D(3,KT)=R*CX
c            write(60,*) x_d,y_d,z_d
            IF(HIPR1(29).EQ.0.0) GO TO 502 
            do KT2=1,KT-1
                DNBP1=(YT_D(1,KT)-YT_D(1,KT2))**2
                DNBP2=(YT_D(2,KT)-YT_D(2,KT2))**2
                DNBP3=(YT_D(3,KT)-YT_D(3,KT2))**2
                DNBP=DNBP1+DNBP2+DNBP3
                IF(DNBP.LT.HIPR1(29)*HIPR1(29)) GO TO 503
            enddo
502        continue 
c        close(60)
        cos_a = 2.0*RANART(NSEED)-1.0
...
c        open(60,FILE='particles.dat',STATUS='UNKNOWN')
        do KT=1,IHNT2(3)
            YT(1,KT)=YT_D(1,KT)*cos_b-YT_D(2,KT)*cos_a*sin_b
     &               +YT_D(3,KT)*sin_a*sin_b

            YT(2,KT)=YT_D(1,KT)*sin_b+YT_D(2,KT)*cos_a*cos_b
     &               -YT_D(3,KT)*sin_a*cos_b
            YT(3,KT)=YT_D(2,KT)*sin_a+YT_D(3,KT)*cos_a
        enddo
        ENDIF
...
C          wuxy helium3
C*****************************
6625        if(IHNT2(3).EQ.3) then
           
           OPEN (527, FILE = './He3list.dat',
     &          STATUS = 'UNKNOWN')
          
           Nhe3=10000
           do index_i =1,Nhe3  
                READ(unit=527,fmt=*) He3_xtot(index_i,1,1),
     &         He3_xtot(index_i,1,2),
     &              He3_xtot(index_i,1,3),He3_xtot(index_i,2,1),
     &              He3_xtot(index_i,2,2),He3_xtot(index_i,2,3),
     &              He3_xtot(index_i,3,1),He3_xtot(index_i,3,2),
     &              He3_xtot(index_i,3,3)
                WRITE(*,*) He3_xtot(index_i,1,1),
     &         He3_xtot(index_i,1,2),
     &              He3_xtot(index_i,1,3),He3_xtot(index_i,2,1),
     &              He3_xtot(index_i,2,2),He3_xtot(index_i,2,3),
     &              He3_xtot(index_i,3,1),He3_xtot(index_i,3,2),
     ...
     &                  + He3_xtot(index_i,index_j,3)*(c2)
           ...
           YT(1,1)= He3_rotation(he3_i,1,1)
           YT(2,1)= He3_rotation(he3_i,1,2)
           YT(3,1)= He3_rotation(he3_i,1,3)
         
           YT(1,2)= He3_rotation(he3_i,2,1)
           YT(2,2)= He3_rotation(he3_i,2,2)
           YT(3,2)= He3_rotation(he3_i,2,3)

           YT(1,3)= He3_rotation(he3_i,3,1)
           YT(2,3)= He3_rotation(he3_i,3,2)
           YT(3,3)= He3_rotation(he3_i,3,3)
...
C          wuxy oxygen NLEFT+
C*****************************
6626        if(IHNT2(3).EQ.16 .AND. nialpha .GT. 0 ) then
           if (nialpha .EQ. 1) then
               OPEN (528, FILE = './NLEFT_dmin_0_5fm_positive_O.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 2) then
               OPEN (528, FILE = './NLEFT_dmin_0_5fm_negative_O.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 3) then
               OPEN (528, FILE = './PGCM_uniform_dmin0_O.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 4) then
               OPEN (528, FILE = './PGCM_clustered_dmin0_O.dat',
     &         STATUS = 'UNKNOWN')
           ...
           do i_oxygen =1,N_oxygen  
              do index_i = 1, IHNT2(3)
                 READ(unit=528,fmt=*) d_ox(i_oxygen,index_i,1),
     &              d_ox(i_oxygen,index_i,2),d_ox(i_oxygen,index_i,3)

c                 write(unit=*,fmt=*) i_oxygen, index_i,
c     &              d_ox(i_oxygen,index_i,1),
c     &              d_ox(i_oxygen,index_i,2),d_ox(i_oxygen,index_i,3)
     ...
     &             + d_O0(index_i,3)*(c2)
           ...
           do index_i = 1, IHNT2(3)
              ...
              YT(1,index_i)= d_O1(index_i,1)
              YT(2,index_i)= d_O1(index_i,2)
              YT(3,index_i)= d_O1(index_i,3) 
...
C          wuxy Neon NLEFT+
C*****************************
6627        if(IHNT2(3).EQ.20 .AND. nialpha .GT. 0 ) then
           if (nialpha .EQ. 1) then
               OPEN (529, FILE = './NLEFT_dmin_0_5fm_positive_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 2) then
               OPEN (529, FILE = './NLEFT_dmin_0_5fm_negative_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 3) then
               OPEN (529, FILE = './PGCM_uniform_dmin0_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           if (nialpha .EQ. 4) then
               OPEN (529, FILE = './PGCM_clustered_dmin0_Ne.dat',
     &         STATUS = 'UNKNOWN')
           ...
           do i_Ne =1,N_Ne  
              do index_i = 1, IHNT2(3)
                 READ(unit=529,fmt=*) d_Nex(i_Ne,index_i,1),
     &              d_Nex(i_Ne,index_i,2),d_Nex(i_Ne,index_i,3)
     ...
     &             + d_Ne0(index_i,3)*(c2)
           ...
           do index_i = 1, IHNT2(3)
              ...
              YT(1,index_i)= d_Ne1(index_i,1)
              YT(2,index_i)= d_Ne1(index_i,2)
              YT(3,index_i)= d_Ne1(index_i,3) 
...
C         OPEN (17, FILE = 'ana/Glauber.txt', STATUS = 'UNKNOWN',
C     &    access='append')

C         write(17, *) '#I IPCOL(I) ITCOL(I) YP(123,JP) YT(123,JT)'
C         DO 71 NCOL=1, NCOLT
C           JP = IPCOL(NCOL)
C           JT = ITCOL(NCOL)
C           write(17, 10501) NCOL, JP, JT, YP(1,JP), YP(2,JP), YP(3,JP),
C     &                     YT(1,JT), YT(2,JT), YT(3,JT)
C71       CONTINUE
C         close(17)

C10501   format (3(I5,2x), 6(e12.6,2x))
...
c.....call ZPC for parton cascade
     ...
     &        XMASS5(I), E5(I)
C The below write is commented by plg on 2011/02/03
C           WRITE (14, 512) PX5(I), PY5(I), PZ5(I), ITYP5(I),
C     &        XMASS5(I), E5(I),LSTRG1(I), LPART1(I), FT5(I)

 1016   CONTINUE
 511    FORMAT(1X, 3F15.6, I4, 2F15.6)
 512    FORMAT(1X, 3F10.3, I6, F6.3,F10.3,1X,I6,1X,I3,1X,F10.3)
...
c        write(17,*) '#b, NATT=', bimp, NATT
c        write(18,*) '#b, NATT=', bimp, NATT
...
C
C
        SUBROUTINE HIJSET(EFRM,FRAME,PROJ,TARG,IAP,IZP,IAT,IZT,NDF)
...
C                        nucleon collisions.
        IF(IHNT2(1).GT.1) THEN
                IF(ABS(NDF-0).LT.0.00001) THEN
                    CALL HIJWDS(IHNT2(1),IHNT2(2),1,RMAX)
                    HIPR1(34)=RMAX
                ENDIF
                IF(ABS(NDF-1).LT.0.00001) THEN
                    CALL HIJWDS_DE(IHNT2(1),1,RMAX)
                    HIPR1(34)=RMAX
                ENDIF
C                        ********set up Wood-Sax distr for proj.
        ...
        IF(IHNT2(3).GT.1) THEN
                IF(ABS(NDF-0).LT.0.00001) THEN
                    CALL HIJWDS(IHNT2(3),IHNT2(4),2,RMAX)
                    HIPR1(35)=RMAX
                ENDIF
                IF(ABS(NDF-1).LT.0.00001) THEN
                    CALL HIJWDS_DE(IHNT2(3),2,RMAX)
                    HIPR1(35)=RMAX
                ENDIF
...
C ********************************************************
C ************************              WOOD-SAX
        SUBROUTINE HIJWDS(IA,IZ,IDH,XHIGH)
...
C     VOL 14, 5-6 1974
        DATA IAA/2,4,12,16,27,32,40,56,63,93,184,197,208,129,6*0./
        DATA RR/0.01,.964,2.355,2.608,2.84,3.458,3.73,3.971,4.214,
     1        4.87,6.51,6.38,6.624,5.42,6*0./
        DATA DD/0.5882,.322,.522,.513,.569,.61,.542,.5935,.586,.573,
     1        .535,.535,.549,0.57,6*0./
        DATA WW/0.0,.517,-0.149,-0.051,0.,-0.208,-0.19,13*0./
...
c wxy 2022.4.6 isobar
        IF(IA .EQ. 96 .AND. IZ .EQ. 40 ) THEN             
c 90 40 Zr
              R = 4.965
              D = 0.556
              W = 0
...
Cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c w-s distribution for deformed nucleus
c
        SUBROUTINE HIJWDS_DE(IA,IDH,XHIGH)
C     SETS UP HISTOGRAM IDH WITH RADII FOR
C     NUCLEUS IA DISTRIBUTED ACCORDING TO THREE PARAM WOOD SAXON
        COMMON/HPARNT/HIPR1(100),IHPR2(50),HINT1(100),IHNT2(50)
cc      HPARNT/
        COMMON/WOODDE/R,D,W,BETA2,BETA4
cc      SAVE /WOOD/
c        DIMENSION IAA(20),RR(20),DD(20),WW(20),RMS(20)
        DIMENSION IAA_D(20),RR_D(20),DD_D(20),WW_D(20),BETA2_D(20),
     &            BETA4_D(20)
        SAVE   
C
C   PARAMETERS OF SPECIAL NUCLEI FROM ATOMIC DATA AND NUC DATA TABLES
C     VOL 14, 5-6 1974
        ...
        DATA DD_D/0.57,19*0./
        DATA WW_D/20*0./
        DATA BETA2_D/0.162,19*0./
        DATA BETA4_D/-0.003,19*0./
C
        A=IA
C
C                 ********SET WOOD-SAX PARAMS FIRST  AS IN DATE ET AL
...
clin RS not used                              RS=RMS(I)
            END IF
        END DO 
        XHIGH=R*(1.0+0.63*BETA2+0.85*BETA4)+12.0*D
C
        IF (IDH.EQ.1) THEN
           HINT1(72)=R
           HINT1(73)=D
...
cccc add by wuxy Feb 13 2019 
c x: r y:cos_eta

        FUNCTION WDSAX_DE(X,Y)
        COMMON/HPARNT/HIPR1(100),IHPR2(50),HINT1(100),IHNT2(50)
        COMMON/WOODDE/R,D,W,BETA2,BETA4
        SAVE   
        
        Y_SQ = Y*Y
        one_over_root_pi = 1.0/(HIPR1(40)**(1./2.))
        Y20 = sqrt(5.)/4.*one_over_root_pi*(3*Y_SQ-1)
        Y40 = 3./16.*one_over_root_pi*(35.*Y_SQ**2-30.*Y_SQ+3.)
        
        REFF= R*(1.0+BETA2*Y20+BETA4*Y40)
        
        WDSAX_DE=1./(1.+EXP((X-REFF)/D)) 
