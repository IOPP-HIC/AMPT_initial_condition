...
cc      SAVE /prec6/
cc      add by plg, Iout(i)=0 if for parton i tau(i) < tau0 
        common /prec7/ tau0, dtau, Iout(MAXPTN)
...
c        open (6, file = 'zpc.res', status = 'unknown')
        OPEN (7, FILE = 'ana/tau0p2.txt', STATUS = 'UNKNOWN')
 ...
 1003   continue
        ...
        if (iconfg .le. 3) then
           do 1004 i = 1, mul
              ...
              if (tau(i).GT.tau0) then
                  Iout(i) = 2
              else if (tau(i).LE.tau0) then
                  Iout(i) = 0
              end if

cc            if the formation time is larger than tau0, do anti-freestreaming
 1004      continue
...
c       this subroutine is used to calculate the 2 particle scattering
        ...
        call partonsAtTau0(t)
cc      Call partonsAtTau0 before position and velocity change
        call newpos(t, iscat)
        call newpos(t, jscat)
        call newmom(t)
...
cc      Add by plg 05/14/2011 to get the partons located between tau0 +- dtau 
        subroutine partonsAtTau0(t)
        implicit double precision (a-h, o-z)
        parameter (MAXPTN=400001)
        common /para1/ mul
cc      SAVE /para1/
cc      updated scattering phase space distribution /prec2/
        common /prec2/gx(MAXPTN),gy(MAXPTN),gz(MAXPTN),ft(MAXPTN),
     &       px(MAXPTN), py(MAXPTN), pz(MAXPTN), e(MAXPTN),
     &       xmass(MAXPTN), ityp(MAXPTN)
cc      SAVE /prec2/
cc      old scattering phase space in /prec3/
        common /prec3/gxs(MAXPTN),gys(MAXPTN),gzs(MAXPTN),fts(MAXPTN),
     &     pxs(MAXPTN), pys(MAXPTN), pzs(MAXPTN), es(MAXPTN),
     &     xmasss(MAXPTN), ityps(MAXPTN)
cc      SAVE /prec3/
...
cc        do 1001 i = 1, MAXPTN
        do 1001 i = 1, mul
            if (Iout(i) .EQ. 0) then 
cc      If this particle's previous tau is still less than tau0
                dt = t-ft(i)
                z = gz(i) + vz(i)*dt
                x = gx(i) + vx(i)*dt
                y = gy(i) + vy(i)*dt
                taup = dsqrt(t**2 - z**2)
                if (taup .gt. tau0) then
                  t0 = Tintp(ft(i), gz(i), t, z, vz(i), tau0)
c                  write(*,*) "wxy1 : ",Iout(i),taup,t
                  dt1 = (t0-ft(i))
                  z0 = gz(i) + vz(i)*dt1
                  x0 = gx(i) + vx(i)*dt1
                  y0 = gy(i) + vy(i)*dt1
                  Iout(i) = 1
cc              write(7, 10500) t0, x0, y0, z0, px(i), py(i), pz(i), e(i)
                  
cc              don't write if e(i)==|pz(i)|, since no solution at tau0
                  if (e(i).GT.abs(pz(i))) then
                    write(7, 10500) e(i),px(i),py(i),pz(i),t0,x0,y0,z0
     &                    ,ityp(i)
                  end if
                end if
c       IF the partons' formation time is larger than tau0, do anti-freestreaming
            else if(Iout(i).EQ.2) then
                  t2 = ft(i)
                  z2 = gz(i)
                  t1 = 0.0d0
                  z1 = 0.0d0
                  vzf = z2/t2
                  if (t2 .LE. abs(z2)) then
                      print *, 'Freestreaming Back Error'
                      print *, 't1=',t1, 'z1=', z1, 't2=',t2,
     &                     'z2=',z2, 'vz=', vz
                  else
                      t0 = Tintp(t1, z1, t2, z2, vzf, tau0)
c                      write(*,*) "wxy2 : ",Iout(i)
                      dt1 = (t0-ft(i))
                      z0 = gz(i) + vzf*dt1
                      x0 = gx(i) + vx(i)*dt1
...
10500   format (8(e15.6, 2x),i10)
        return
        end


        function Tintp(t1, z1, t2, z2, vz, tau0)
cc      calc t for where a parton pass though tau0 line from (t1,z1) to (t2,z2)
cc      with speed vz
        implicit double precision (a-h, o-z)
        a = 1.0 - vz*vz
        b = z1 - vz*t1
        Tintp1 = ( b*vz + dsqrt(tau0*tau0*a + b*b) )/a
        Tintp2 = ( b*vz - dsqrt(tau0*tau0*a + b*b) )/a
        if ( (Tintp1.GE.t1).AND.(Tintp1.LE.t2) ) then
            Tintp = Tintp1
        else if ( (Tintp2.GE.t1).AND.(Tintp2.LE.t2) ) then
            Tintp = Tintp2
        else 
            print *, 'plg say: No solution in finding t!'
            print *, 't1=', t1, 'z1=', z1, 't2=', t2, 'z2=', z2,
     &               'vz=', vz, 'tau0=', tau0
        end if
        return
        end
