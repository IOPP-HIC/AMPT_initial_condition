...
C     hjcrdn: Used to print the position of jet partons      
      COMMON/hjcrdn/YP(3,300),YT(3,300)
C     HSTRNG: NFP(K,6)=is the total number of jet production associated
C     with ths projectile. if NFP(K,6)<0 it can not produce jet anymore
C     due to energy conservation
C             NFT(K,6)= the total number of jet production associated
C             with this target
...
C**** IHPR2(8) default = 10; the maximum number of jet production;
C      IHPR2(8)=0
...
c     AMPT momentum and space info at freezeout:
      ...
      OPEN (20, FILE = 'ana/jet_distribution.dat', STATUS = 'UNKNOWN')
...
c      OPEN (18, FILE = 'ana/res-loss.dat', STATUS = 'UNKNOWN')
      CALL HIJSET(EFRM, FRAME, PROJ, TARG, IAP, IZP, IAT, IZT,NDF)
      CALL ARTSET
      CALL INIZPC
c
       DO 2000 J = 1, NEVNT
...
c         write(20, *) '#num_of_jet_pairs  x   y'
C.....IAP: mass number of the projectile
         DO 71 K=1, IAP
c           write(20, 10501) 0.5*NFP(K,6), YP(1, K), YP(2, K)
c           write(20, 10501) 0.5*NFT(K,6), YT(1, K), YT(2, K)
71       CONTINUE
