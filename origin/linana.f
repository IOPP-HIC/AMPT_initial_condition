...
c     ONLY use one data set for analysis to avoid double-counting:
               if(dplast(ip).gt.oneminus.and.dplast(ip).lt.oneplus) then
c                  write(16,200) INVFLV(lblast(ip)), plast(1,ip),
c     1                 plast(2,ip),plast(3,ip),plast(4,ip),
c     2                 xlast(1,ip),xlast(2,ip),xlast(3,ip),
c     3                 1.E-20*sqrt(plast(3,ip)**2+plast(4,ip)**2)
c     4                 /plast(4,ip)
clin-12/14/03-end
               else
...
c            write(16,200) INVFLV(lblast(ip)), plast(1,ip),
c     1           plast(2,ip),plast(3,ip),plast(4,ip),
c     2           xlast(1,ip),xlast(2,ip),xlast(3,ip),xlast(4,ip)
               else
                  if(idpert.eq.1.or.idpert.eq.2) then
            write(90,250) INVFLV(lblast(ip)),plast(1,ip),
     1           plast(2,ip),plast(3,ip),
...
c     change format for large numbers:
               if(dplast(ip).gt.oneminus.and.dplast(ip).lt.oneplus) then
c            write(16,201) INVFLV(lblast(ip)), plast(1,ip),
c     1           plast(2,ip),plast(3,ip),plast(4,ip),
c     2           xlast(1,ip),xlast(2,ip),xlast(3,ip),xlast(4,ip)
               else
                  if(idpert.eq.1.or.idpert.eq.2) then
...
c           write(92,*) 3*nsmbbbar+2*nsmmeson
c           write(92,*) ' is the total # of initial partons after string 
c     1 melting'
